...
interface CalculatorCoordinator {
    ...
    val expressionStringAdapter: ExpressionStringAdapter //todo remove after
}
...
class CalculatorCoordinatorImpl: CalculatorCoordinator { //todo weird name
    ...
    override val expressionStringAdapter: ExpressionStringAdapter = ExpressionStringAdapterImpl(expressionBuilder, false, true)
...
//            }
    ...
    private val expressionBuilderListener = object: ExpressionBuilder.Listener {
        override fun expressionWasCleared() {
            customEditText?.setText(expressionStringAdapter.expressionToString(), TextView.BufferType.EDITABLE)
        }

        override fun exprTokenWasAddedAt(token: ExpressionToken, index: Int) {
            customEditText?.setText(expressionStringAdapter.expressionToString(), TextView.BufferType.EDITABLE)
            customEditText?.setSelection(expressionStringAdapter.expressionIndexToStringIndex(index+1))
        }
        ...
        override fun exprTokenWasRemovedAt(token: ExpressionToken, index: Int) {
            customEditText?.setText(expressionStringAdapter.expressionToString(), TextView.BufferType.EDITABLE)
            customEditText?.setSelection(expressionStringAdapter.expressionIndexToStringIndex(index))
        }
    }
    ...
    private fun fixSelectionPositionByConvertingToExpressionAndBackToString(selectionPosition: Int): Int {
        return expressionStringAdapter.expressionIndexToStringIndex(
            expressionStringAdapter.stringIndexToExpressionIndex(selectionPosition))
    }
